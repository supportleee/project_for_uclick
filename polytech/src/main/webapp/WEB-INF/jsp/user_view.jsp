...
<body
	onload="onloadSelect('${user.department}', '${user.team }', '${user.rank }')">
				...
				<tr>
					...
					<td class="align-middle">
						<button type="button" class="btn btn-secondary"
							data-toggle="modal" data-target="#user_modifyModal">수정</button>
					</td>
					<td>
						<form method="POST"
							action="/user_delete/${user.id}?_method=DELETE">
	...
	<!-- Modal -->
	<div class="modal fade" id="user_modifyModal" tabindex="-1"
		role="dialog" aria-labelledby="user_modifyModalLabel"
		aria-hidden="true">
					...
					<div class="modal-body">
						<div class="form-group">
							<label for="name" class="col-form-label">이름 :</label> <input
								...
								maxlength="20" value="${user.name }" required>
						</div>
						<div class="form-group">
		...
		<div class="row col-8 p-0 justify-content-between">
			...
			<div class="m-0 row align-items-center">
				<button type="button" class="btn btn-secondary my-1"
					data-toggle="modal" data-target="#phone_insertModal">추가</button>
			</div>
		</div>
	</div>
	<!-- Modal -->
	<div class="modal fade" id="phone_insertModal" tabindex="-1"
		role="dialog" aria-labelledby="phone_insertModalLabel"
		aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
				...
				<form method="post" action="../phone_save/${user.id }">
					<div class="modal-body">
						<div class="form-group">
							<label for="type" class="col-form-label">종류 :</label> <select
						...
						<div class="form-group">
							<label for="tel" class="col-form-label">전화번호 :</label><input
					...
					<c:otherwise>
						<c:forEach items="${phones }" var="phone">
							<tr>
								...
								<td class="align-middle"><button type="button" class="btn btn-secondary"
										data-toggle="modal" data-target="#phone_modifyModal">수정</button>
									<!-- Modal -->
									<div class="modal fade justify-content-start" id="phone_modifyModal" tabindex="-1"
										role="dialog" aria-labelledby="phone_modifyModalLabel"
										aria-hidden="true">
										<div class="modal-dialog" role="document">
											<div class="modal-content">
												<div class="modal-header">
													<h5 class="modal-title" id="phone_modifyModalLabel">전화기
														수정</h5>
													<button type="button" class="close" data-dismiss="modal"
														aria-label="Close">
														<span aria-hidden="true">&times;</span>
												...
												<form method="post" action="../phone_save/${user.id }">
													<div class="modal-body justify-content-start">
														<div class="form-group">
															<label for="type" class="col-form-label">종류 :</label> <select
																class="form-control" id="type" name="type" required>
																<option value="" selected disabled hidden>종류를
																	선택하세요.</option>
																<option value="모바일"
																	<c:if test="${phone.type eq '모바일' }">selected</c:if>>모바일</option>
																<option value="IP전화"
																	<c:if test="${phone.type eq 'IP전화' }">selected</c:if>>IP전화</option>
																<option value="FAX"
																	<c:if test="${phone.type eq 'FAX' }">selected</c:if>>FAX</option>
															</select>
														</div>
														<div class="form-group">
															<label for="tel" class="col-form-label">전화번호 :</label><input
																type="text" class="form-control" id="tel" name="tel"
																value="${phone.tel }" maxlength="11" required>
														</div>
													</div>
													<div class="modal-footer">
														<button type="button" class="btn btn-secondary"
								...
								<td>
									<form method="POST"
										action="/phone_delete/${phone.id}?_method=DELETE">
										<button type="submit" class="btn btn-secondary">삭제</button>
									</form>
								</td>
							</tr>
